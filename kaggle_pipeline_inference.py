...
def _apply_strict_filters(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if removed_exp > 0:
        print(f"   Removed {removed_exp} fights from split decisions or women's divisions")
...
def _compute_ratio_features(long_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Best proxy: 1 - (opp_subs_landed_cum / opp_subs_attempted_cum). 
    # But usually subs landed = 1 (fight driver). So we use: 1 / (opp_sub_attempts + 1) NO.
    # Let's stick to simple "opp_sub_attempts" (lower is better defense).
    # But wait, we can compute "opp_sub_success_rate" inverse.
    if 'submission_attempts_dec_avg' in long_df.columns:
        opp_sub = long_df[['fight_id', 'fighter_name', 'submission_attempts_dec_avg']].copy()
        opp_sub.columns = ['fight_id', 'opponent_name', 'opp_sub_attempts']
        long_df = long_df.merge(opp_sub, on=['fight_id', 'opponent_name'], how='left')
        # sub_def: Lower opp attempts is better, but hard to normalize. 
        # Feature request was sub_def_dec_adjperf... imply there is a base metric.
        # Let's define sub_def as: 1 / (1 + opp_sub_attempts_per_min)
        # We need opp_ctrl_time as normalizer or fight_time.
        pass # Skipping complex sub_def for now, reliance on grappling_defense_total (td_def) is better.
...
def _compute_history_features(long_df: pd.DataFrame, inference_mode=False) -> pd.DataFrame:
    """
    ...
    Args:
        inference_mode: If True, disables shift(1) to include latest fight data.
    """
    long_df = long_df.copy()
    ...
    def _extract_opponent_stats(df, stats_to_extract):
        ...
        opp = df[['fight_id', 'fighter_name'] + stats_to_extract].copy()
        
        # Rename so that fighter_name -> opponent_name for the merge
        opp.columns = ['fight_id', 'opponent_name'] + [f'opp_{col}' for col in stats_to_extract]
    ...
    # If inference_mode=True, we DO NOT SHIFT.
    # We want Row[N] to contain stats AFTER fight N, so it can be used to predict Fight N+1.
    shift_val = 0 if inference_mode else 1
    
    for col in numeric_cols:
        # Compute ALL layers (needed internally)
        long_df[f'{col}_career_avg'] = grouped[col].transform(
            lambda x: x.shift(shift_val).expanding().mean()
        ).fillna(0)
        
        long_df[f'{col}_dec_avg'] = grouped[col].transform(
            lambda x: x.shift(shift_val).ewm(alpha=0.15, min_periods=1).mean()
        ).fillna(0)
        
        # roll3 computation REMOVED - not used in final features
    # Win Rate - LEAKAGE SAFE: shift(shift_val) ensures current fight outcome is EXCLUDED in training
    ...
    long_df['career_wins'] = grouped['win'].transform(lambda x: x.shift(shift_val).expanding().sum()).fillna(0)
    long_df['career_fights'] = grouped['win'].transform(lambda x: x.shift(shift_val).expanding().count()).fillna(0)
    ...
    def compute_streak(wins):
        shifted = wins.shift(shift_val).fillna(0)
        streak = []
        current_streak = 0
        for w in shifted:
            if w == 1:
    ...
    if 'ko_tko_win' in long_df.columns:
        long_df['career_ko_wins'] = grouped['ko_tko_win'].transform(lambda x: x.shift(shift_val).expanding().sum()).fillna(0)
    ...
    if 'decision_win' in long_df.columns:
        long_df['career_dec_wins'] = grouped['decision_win'].transform(lambda x: x.shift(shift_val).expanding().sum()).fillna(0)
...
def _compute_opponent_adjusted_features(long_df: pd.DataFrame, inference_mode=False) -> pd.DataFrame:
    ...
    for base in base_stats:
        if base in long_df.columns:
            ...
            wc_median = long_df.groupby(wc_grouper)[base].transform(
                lambda x: x.expanding().median().shift(shift_val)
            )
            ...
            # Or just expanding STD * 0.6745.
            # Let's go with expanding STD * 0.6745 as a high-quality proxy for MAD in streaming data.
            # It preserves the scale and is O(N).
            
            wc_mad = long_df.groupby(wc_grouper)[base].transform(
                 lambda x: x.expanding().std().shift(shift_val) * 0.6745
            )
            ...
            # Safe padding:
            wc_median = wc_median.fillna(0)
            wc_mad = wc_mad.fillna(1.0).replace(0, 1e-6)
            
            long_df[f'{base}_wc_mean'] = wc_median
            long_df[f'{base}_wc_mad'] = wc_mad
    ...
    for base in base_stats:
        ...
        # Opponent MAD (Rolling 5) - Shifted
        opp_stats[f'{base}_opp_mad'] = grouped_opp[col_allowed].transform(
            lambda x: x.shift(shift_val).rolling(window=5, min_periods=1).apply(
    ...
    for base in base_stats:
        ...
        if obs_col in long_df.columns and opp_mean_col in long_df.columns:
            ...
            mad_opp = long_df[opp_mad_col].fillna(long_df[wc_mad_col])
            
            mu_shrunk = (w * mu_opp) + ((1 - w) * long_df[wc_mean_col])
            
            # Shrunk MAD
            # Floor MAD to prevent division by zero or extreme sensitivity
            mad_shrunk = (w * mad_opp) + ((1 - w) * long_df[wc_mad_col])
            mad_shrunk = mad_shrunk.replace(0, 1e-6)
            
            # Robust Z-Score of the ACTUAL fight performance
            z_col = f'{base}_zscore_raw'
            long_df[z_col] = (long_df[base] - mu_shrunk) / mad_shrunk
            long_df[z_col] = long_df[z_col].clip(-7, 7)

    # === FINAL LAYER: Decayed Average of Z-Scores ===
    grouped = long_df.groupby('fighter_name', group_keys=False)
    for base in base_stats:
        z_col = f'{base}_zscore_raw'
...
def _compute_elo_ratings(long_df: pd.DataFrame, inference_mode=False) -> pd.DataFrame:
    """
    ...
    post_match_elo_values = np.full(n_rows, np.nan)  # For inference mode
    ...
    for fight_id in fight_ids:
        ...
        # Store POST-MATCH Elo for inference
        if idx_a is not None:
            post_match_elo_values[idx_a] = new_elo_a
        if idx_b is not None:
            post_match_elo_values[idx_b] = new_elo_b
    ...
    # Add to dataframe
    # If inference_mode, use POST-MATCH Elo (new_elo) as the fighter's current state
    if inference_mode:
        long_df['elo'] = post_match_elo_values
    else:
...
# =============================================================================
# DATA PREPROCESSING FUNCTIONS (Added for Utility Script Compatibility)
# =============================================================================
...
def randomize_fighter_positions(df, seed=42):
    ...
    if 'outcome' in df.columns:
        ...
        print(f'Randomized {swapped_rows} rows for 50/50 balance')
